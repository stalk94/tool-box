...
const resolveFetch$3 = (customFetch) => {
  ...
  if (customFetch) {
    ...
    _fetch = (...args) => Promise.resolve().then(() => browser).then(({ default: fetch2 }) => fetch2(...args));
  } else {
    _fetch = fetch;
  }
  ...
  electron.ipcMain.handle("auth:google", async () => {
    try {
      ...
      if (error || !(data == null ? void 0 : data.url)) {
        console.error("❌ Supabase ошибка:", error == null ? void 0 : error.message);
        return { success: false, error: (error == null ? void 0 : error.message) || "No auth URL returned" };
      }
      const win = new electron.BrowserWindow({
        width: 500,
        height: 700,
